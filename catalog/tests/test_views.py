...
class AuthorListViewTest(TestCase):
    ...
    def test_view_uses_correct_template(self):
        response = self.client.get(reverse('authors'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'author/authors_list.html')
...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        self.assertTemplateUsed(response, 'book/bookinstance_list_borrowed_user.html')
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_uses_correct_template(self):
        ...
        # Check we used correct template
        self.assertTemplateUsed(response, 'book/book_renew.html')
...
class AuthorCreateTest(TestCase):
    ...
    def setUp(self):
        ...
        permission = Permission.objects.get(name='May create, edit and delete authors')
        test_user2.user_permissions.add(permission)
        test_user2.save()
    ...
    def test_logged_in_and_correct_permission(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_author_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-create'), {'first_name':'John', 'last_name':'Smith'})
        self.assertEqual(response.status_code, 302)

        self.assertTrue(response.url.startswith('/catalog/author/1'))
...
class AuthorUpdateTest(TestCase):
    ...
    def test_initial_author_first_name_generated(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-update', kwargs={'pk': self.test_author.pk}))
        self.assertEqual(response.status_code, 200)
        
        # Check that the initial update author data is used
        self.assertEqual(response.context['form'].initial['first_name'], self.test_author.first_name)

    def test_initial_author_last_name_generated(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-update', kwargs={'pk': self.test_author.pk}))
        self.assertEqual(response.status_code, 200)

        # Check that the initial update author data is used
        self.assertEqual(response.context['form'].initial['last_name'], self.test_author.last_name)

    def test_redirects_to_author_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author-update', kwargs={'pk': self.test_author.pk}), {'first_name':"Jimothy",'last_name':'Jamborie'})
        self.assertEqual(response.status_code, 302)

        self.assertRedirects(response, reverse('author-detail', kwargs={'pk': self.test_author.pk}))
